...
/*Görev 1: macSkoru()
  ...
  1. skor1 ve skor2 arasındaki fark nedir?
    ilkinde skor değişkenine global'de erişemiyoruz.
  2. Hangisi bir closure kullanmaktadır? Nasıl tarif edebilirsin? (yarınki derste öğreneceksin :) )
    ilkinde.en genel haliyle function içinde function returnlenir.
  3. Hangi durumda skor1 tercih edilebilir? Hangi durumda skor2 daha mantıklıdır?
  skor 1 takım skorlarını tutmak için mantıklıdır. skor2'deki skor değişkeni daha genel bir yaklaşımdır.
*/
...
function skorTabelasi(cbPeriod, cbScore, q) {
  ...
  let scoreBoard = [];
  for (let i = 0; i < q; i++) {
    const periyotResult = cbPeriod(cbScore);
    console.log("periodResult", i, periyotResult);
    home = home + periyotResult.evSahibi;
    away = away + periyotResult.konukTakım;

    const periyotResultString = `${i + 1}. Periyot: Ev Sahibi ${
      periyotResult.evSahibi
    }- Konuk Takım ${periyotResult.konukTakım}`;
    scoreBoard.push(periyotResultString);
  }
